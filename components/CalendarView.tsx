...
const CalendarView: React.FC<CalendarViewProps> = ({ user, attendance, dayAttendance, timetable, onMarkDayAttendance, onMarkSubjectAttendance }) => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-6 duration-700">
      <div className="card-rich p-8 bg-slate-900/60 border-white/20">
        <div className="flex justify-between items-center mb-10">
          <button onClick={handlePrev} className="w-12 h-12 flex items-center justify-center rounded-2xl bg-white/10 text-white active:scale-90 border border-white/20">
          ...
          <div className="text-center">
            ...
            <span className="text-xs font-black text-indigo-400 uppercase tracking-[0.4em]">{year}</span>
          </div>
          <button onClick={handleNext} className="w-12 h-12 flex items-center justify-center rounded-2xl bg-white/10 text-white active:scale-90 border border-white/20">
        ...
        <div className="grid grid-cols-7 mb-6">
          {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map(d => (
            <div key={d} className="text-center text-[10px] font-black text-indigo-200 uppercase tracking-widest">{d}</div>
          ))}
        </div>
          ...
          {days.map((date, idx) => {
            ...
            return (
              <button 
                ...
                className={`relative h-12 flex flex-col items-center justify-center rounded-2xl transition-all duration-300 text-sm font-black
                  ${isSel ? 'bg-white text-indigo-950 shadow-xl' : 'text-white hover:bg-white/10'}
                  ${isToday && !isSel ? 'ring-2 ring-indigo-400 text-indigo-300' : ''}
                  ${isFut ? 'opacity-20 cursor-not-allowed' : 'active:scale-90'}
                `}
              >
                {date.getDate()}
                {rec && !isSel && (
                  <div className={`absolute bottom-2 w-1.5 h-1.5 rounded-full ${
                    rec.status === 'PRESENT' ? 'bg-emerald-400 shadow-[0_0_8px_emerald]' : 
                    rec.status === 'ABSENT' ? 'bg-rose-400 shadow-[0_0_8px_rose]' : 'bg-amber-400 shadow-[0_0_8px_amber]'
                  }`}></div>
                )}
              </button>
            );
      ...
      <div className="space-y-6 pt-2">
        <div className="flex justify-between items-center px-4">
          <div>
            <span className="text-[10px] font-black text-indigo-200 uppercase tracking-widest">Selected Date</span>
            <h3 className="text-3xl font-black text-white tracking-tighter">
              {selectedDate.toLocaleDateString('default', { day: 'numeric', month: 'short' })}
            </h3>
          </div>
          <div className={`px-5 py-2.5 rounded-2xl text-[10px] font-black uppercase tracking-widest border border-white/30 bg-black/40 text-white`}>
        ...
        {user.useAdvancedMode && daySlots.length > 0 && (
          <div className="space-y-5 pt-6 pb-12">
            <h4 className="text-xs font-black text-indigo-200 uppercase tracking-widest px-4">Subject Registry</h4>
            <div className="space-y-4">
              {daySlots.map((item, i) => (
                <div key={item.id || i} className="card-rich p-6 bg-slate-900/60 border-white/20 space-y-6 shadow-xl">
                  <div className="flex justify-between items-center">
                    <h5 className="text-xl font-black text-white tracking-tighter">{item.name}</h5>
                    <span className={`text-[10px] font-black uppercase tracking-widest px-4 py-1.5 rounded-full border border-white/20 bg-black/40 text-white`}>
                      {item.status || 'Mark'}
                    </span>
                  </div>
...
const ActionBtn: React.FC<{ active: boolean; color: string; icon: string; label: string; onClick: () => void }> = ({ active, color, icon, label, onClick }) => {
  const styles: any = {
    emerald: active ? 'bg-emerald-600 border-white shadow-xl' : 'bg-emerald-500/10 border-white/10 text-emerald-400',
    rose: active ? 'bg-rose-600 border-white shadow-xl' : 'bg-rose-500/10 border-white/10 text-rose-400',
    amber: active ? 'bg-amber-600 border-white shadow-xl' : 'bg-amber-500/10 border-white/10 text-amber-400'
  };
  return (
    <button onClick={onClick} className={`flex flex-col items-center justify-center py-8 rounded-[2.5rem] transition-all duration-300 border-2 active:scale-95 text-white ${styles[color]}`}>
...
const MiniActionBtn: React.FC<{ active: boolean; color: string; icon: string; onClick: () => void }> = ({ active, color, icon, onClick }) => {
  const styles: any = {
    emerald: active ? 'bg-emerald-600 border-white' : 'bg-black/40 border-white/10 text-emerald-400',
    rose: active ? 'bg-rose-600 border-white' : 'bg-black/40 border-white/10 text-rose-400',
    amber: active ? 'bg-amber-600 border-white' : 'bg-black/40 border-white/10 text-amber-400'
  };
  return (
    <button onClick={onClick} className={`flex-1 h-14 rounded-2xl border-2 flex items-center justify-center transition-all duration-300 active:scale-95 text-white ${styles[color]}`}>
